...
def adjust_temperature(epoch, warmup_epochs, total_epochs, ITERS_PER_EPOCH,
                       min_temp=20, max_temp=20, iters=-1):
    if epoch < warmup_epochs:
        return min_temp  # 预热阶段保持低温

    if epoch >= total_epochs:
        return max_temp  # 超过总 epoch 后保持最高温
    ...
    total_iters = ITERS_PER_EPOCH * (total_epochs - warmup_epochs)
    iters = iters - ITERS_PER_EPOCH * warmup_epochs
    temperature = min_temp + (max_temp - min_temp) * (1 - (math.cos(iters / total_iters * math.pi) + 1) * 0.5)
