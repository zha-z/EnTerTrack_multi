...
class UAVDTDataset(BaseDataset):
    ...
    def _construct_sequence(self, sequence_name):
        anno_path = '{}/UAV-benchmark-SOT_v1.0/{}/{}_gt.txt'.format(self.base_path, 'anno',sequence_name)   #'{}/{}/{}_gt.txt'

        ground_truth_rect = load_text(str(anno_path), delimiter=',', dtype=np.float64)

        frames_path = '{}/{}/{}'.format(self.base_path, 'sequences',sequence_name)
        frame_list = [frame for frame in os.listdir(frames_path) if frame.endswith(".jpg")]
        #frame_list = [frame for frame in (frames_path) if frame.endswith(".jpg")]    #?没有jpg文件名储存进来
        frame_list.sort(key=lambda f: int(f[3:-4]))  #根据jpg序号排列，framelist存储字符串格式“img0000x.jpg"取第四个字符到倒数第四个字符
