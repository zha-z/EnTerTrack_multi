...
class VisionTransformerCE(VisionTransformer):
    ...
    def forward_features(self, z, x, mask_z=None, mask_x=None,
        ...
        global_index_t = torch.arange(lens_z, device=x.device).unsqueeze(0).repeat(B, 1)
        ...
        for i, blk in enumerate(self.blocks):
            x, global_index_s, removed_index_s, attn, atp_mask, attn_mask, frozen_token = \
                blk(x, global_index_t, global_index_s, attn_mask, ce_template_mask, training, ce_keep_rate,
                    temperature=temperature, frozen_token=frozen_token)
        ...
        if frozen_token is not None:
            ...
            x, _ = self._recover_frozen_tokens(
                x, frozen_token, curr_atp_mask, curr_removed_idx,
                global_index_s, lens_z, training
            )
        elif removed_indexes_s and removed_indexes_s[-1] is not None and lens_x_new != 256:
    ...
    def forward(self, z, x, ce_template_mask=None, ce_keep_rate=None,
        ...
        x, aux_dict = self.forward_features(z, x, ce_template_mask=ce_template_mask, ce_keep_rate=ce_keep_rate,
                                            training=training, return_atp=return_atp, temperature=temperature)

        return x, aux_dict

    def fcvc_capture_taps(self, z, x, mid_after_block=3):
        from .fcvc.feature_taps import capture_taps

        return capture_taps(self, z, x, mid_after_block=mid_after_block)

    def fcvc_replay_blocks(self, tokens, start_block=4, end_block=6):
        from .fcvc.feature_taps import replay_blocks

        return replay_blocks(self, tokens, start_block=start_block,
                             end_block=end_block)
