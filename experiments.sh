...
#   bash experiments.sh test_motion
...
TEST_MOTION_CONFIGS=(
  pcum_ablation_current_full_remote_motion_redetect
)
...
run_test_motion() {
  for cfg in "${TEST_MOTION_CONFIGS[@]}"; do
    python tracking/test.py \
      --tracker_name entertrack \
      --tracker_param "${cfg}" \
      --dataset_name "${DATASET}" \
...
run_smoke_motion() {
  python tracking/test.py \
    --tracker_name entertrack \
    --tracker_param pcum_ablation_current_full_remote_motion_redetect \
    --dataset_name "${DATASET}" \
    --sequence 0 \
    --runid "${RUNID_SMOKE}" \
    --threads 0 \
...
case "${MODE}" in
  ...
  *)
    ...
    echo "Valid modes: train, test_local, test_remote, test_motion, test_all, smoke, smoke_motion, analysis" >&2
    exit 2
    ;;
esac
